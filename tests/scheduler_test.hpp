...
TEST(scheduler, create_scheduler) {
  ...
  scheduler.ScheduleAt(std::chrono::steady_clock::now() + 25ms,
                       [&counter]() { counter++; });
...
namespace {
bool TimeIsNear(klyaksa::Timepoint lhs, klyaksa::Timepoint rhs,
                klyaksa::Timeout error) noexcept {
...
TEST(scheduler, schedule_at_expired_time) {
  ...
  scheduler.ScheduleAt(
      ...
      [&exec_time]() { exec_time = std::chrono::steady_clock::now(); });
  ...
  ASSERT_TRUE(TimeIsNear(exec_time, now, error))
      ...
      << std::chrono::duration_cast<klyaksa::Timeout>(exec_time - now).count()
      << "| <= " << error.count();
  ASSERT_EQ(scheduler.CallbackCount(), 0);
}
...
TEST(scheduler, schedule_at_future_time) {
  ...
  const auto diff = std::chrono::duration_cast<klyaksa::Timeout>(
      *exec_time - expectedExecTime);
  ASSERT_TRUE(TimeIsNear(*exec_time, expectedExecTime, error))
      << "Executed at " << exec_time->time_since_epoch().count()
      << " expected execution around "
      << expectedExecTime.time_since_epoch().count()
      << " so |exec_time -  now| <= error: |" << diff.count()
      << "| <= " << error.count();
}
...
TEST(scheduler, correct_schedule_after_sequence) {
  ...
  for (size_t i = 0; i < kInsertElements; i++) {
    ...
    scheduler.ScheduleAfter(timeout,
