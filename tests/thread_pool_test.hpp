...
TEST(thread_pool, post_lambdas) {
  ...
  auto results = {Post(executor, [] { std::this_thread::sleep_for(5ms); }),
                  Post(executor,
                       [&count] {
                         count++;
                  ...
                  Post(
                      executor,
                      [&count](int x, int y) mutable {
                        count += x + y;
                        std::this_thread::sleep_for(5ms);
                      },
                      1, 2)};
  auto res = Post(executor, [&count] {
    count++;
    std::this_thread::sleep_for(5ms);
    return count.load();
  });
...
TEST(thread_pool, task_throw_exception) {
  ...
  klyaksa::ThreadPool executor{kWorkers};
  executor.Start();
  auto fut = Post(executor, []() { throw std::runtime_error("test error"); });
  fut->wait();
